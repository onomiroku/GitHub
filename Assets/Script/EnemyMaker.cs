...
public class EnemyMaker : MonoBehaviour
{
    ...
    void Update()
    {
        if(waitTime < makeTime) //敵が現れるまでの時間を数える。
        {
        ...
        else
        {
            ...
            Instantiate(enemyPrefab, new Vector3(ranX, 0, enemyZ), enemyPrefab.transform.rotation); //オブジェクトを出現させる（enemyPrefabをvector3の座標で、enemyPrefabのrotationで）
            waitTime = 0;
        } 
          //waitTimeを0にすることで、もう一回
    }
}
